...
public class AsteraX : MonoBehaviour
{
    ...
    // Noves variables per als nivells
    static private int             _LEVEL = 0;
    static public int              LEVEL { get { return _LEVEL; } private set { _LEVEL = value; } }
    ...
    static public int NumAsteroidChildren {
        get { return _numAsteroidChildren; }
        set { _numAsteroidChildren = value; }
    }
    ...
    public enum eGameState
    {
        ...
        paused = 32,    // 00100000 (nou estat per pausar el joc)
    ...
    // Mètode modificat per començar amb el menú principal en lloc de generar asteroides directament
    void Start()
    {
#if DEBUG_AsteraX_LogMethods
        ...
        // En lloc de generar asteroides aquí, començarem amb l'estat de menú principal
        GAME_STATE = eGameState.mainMenu;
    }

    // Mètode per iniciar el joc des del menú principal
    static public void StartGame() {
        if (_S != null) {
            SCORE = 0; // Reiniciar la puntuació
            _S.StartLevel(1); // Començar amb el nivell 1
        }
    }

    // Mètode modificat per iniciar un nivell específic
    public void StartLevel(int levelNum) {
        ...
        // Eliminar qualsevol asteroide existent
        if (ASTEROIDS != null) {
            foreach (Asteroid ast in ASTEROIDS.ToArray()) {
        ...
        // Reiniciar les llistes
        ASTEROIDS = new List<Asteroid>();
        BULLETS = new List<Bullet>();
        
        // Generar els asteroides pare pel nivell
        for (int i = 0; i < _numInitialAsteroids; i++) {
    ...
    static public string LevelProgression {
        get {
            if (S != null) {
                return S.levelProgression;
            }
            return "";
        }
    }

    // Nou mètode per analitzar la configuració de nivells
    void ParseLevelProgression(int levelNum) {
            ...
            if (parts.Length == 2) {
                ...
                if (currentLevel == levelNum) {
                    ...
                    if (settings.Length == 2) {
                        _numInitialAsteroids = int.Parse(settings[0]);
                        _numAsteroidChildren = int.Parse(settings[1]);
                        return;
                    }
                }
            }
        ...
        _numAsteroidChildren = 2;
    }

    // Nou mètode per comprovar quan s'ha completat un nivell
    ...
    // Nou mètode per avançar al següent nivell
    void AdvanceToNextLevel() {
        if (LEVEL < 10) {
            LEVEL++;
            StartLevel(LEVEL);
        } else {
            // Si hem completat tots els nivells, finalitzar el joc
            GameOver();
        }
    }

    // Nou mètode per canviar entre pausa i joc actiu
    static public void TogglePause(bool pauseState) {
        if (pauseState && GAME_STATE == eGameState.level) {
            GAME_STATE = eGameState.paused;
            Time.timeScale = 0f;
        } else if (!pauseState && GAME_STATE == eGameState.paused) {
            GAME_STATE = eGameState.level;
            Time.timeScale = 1f;
        }
    }

    // Mètode modificat per gestionar el final del joc
    public void EndGame() {
        GAME_STATE = eGameState.gameOver;
    }

    // Mètode modificat per tornar al menú principal
    ...
    // Mètode antic que reinicia l'escena (mantingut per compatibilitat)
    void ReloadScene()
    {
    ...
    // Nous mètodes per gestionar les bales
    ...
    static public void RemoveBullet(Bullet bullet) {
        if (BULLETS != null && BULLETS.IndexOf(bullet) != -1) {
            BULLETS.Remove(bullet);
        }
    }

    // Nou mètode a Update per comprovar la finalització del nivell
    void Update() {
